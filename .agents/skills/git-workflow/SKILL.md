...
- For implementation work, use a Codex-prefixed feature branch unless the user explicitly asks to work in the current checkout.
- Do not push, open a pull request, or otherwise modify remote state unless the user explicitly asks.
...
- Put task worktrees next to the repository root by default. For example, `agent-harness` branch `codex/feat/worktree-branch-delivery` uses `../agent-harness-codex-feat-worktree-branch-delivery`.
...
- `codex/` is required for branches created by Codex.
...
```text
codex/feat/jwt-refresh-token-rotation
codex/fix/parser-empty-input
codex/refactor/extract-query-builder
codex/docs/clarify-install-steps
```
...
5. Only when the user explicitly asks for delivery, push the feature branch and create a pull request:
   - `git fetch origin`
